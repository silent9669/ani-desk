...
use ani_desk_core::skip_times::SkipTime;
use anyhow::Context;
use chrono::Utc;
...
#[tauri::command]
async fn get_skip_times(input: SkipTimesInput) -> Result<Vec<SkipTime>, AppErrorDto> {
    ani_desk_core::skip_times::fetch_skip_times(input.catalog_id, input.episode_number)
        .await
        .map_err(|error| app_error("SKIP_TIMES_UNAVAILABLE", "skip-times", None, error, true))
}
