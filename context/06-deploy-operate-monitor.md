...
This runbook describes the existing Docker Compose/systemd model. Replace the example domain and paths only through the documented environment variables. Do not run deployment until design/provider validation is approved.

## Host layout

```text
/srv/ani-desk/source     isolated deployment checkout
...
```sh
sudo install -d -o dangphuc -g docker -m 0750 /srv/ani-desk/{source,data,backups,state,deployer}
...
```sh
docker compose -p homelab -f /srv/ani-desk/source/deploy/homelab/compose.yml ps
docker compose -p homelab -f /srv/ani-desk/source/deploy/homelab/compose.yml logs --tail=150 ani-desk
docker compose -p homelab -f /srv/ani-desk/source/deploy/homelab/compose.yml logs --tail=150 caddy
curl -fsS https://YOUR_DOMAIN/api/health
systemctl status ani-desk-deploy.timer --no-pager
...
Install reviewed copies of `pull-deploy.sh`, `ani-desk-deploy.service`, and `ani-desk-deploy.timer` into the paths named by the unit, then:
...
The agent deploys only when the latest completed GitHub `CI` push run on the configured branch succeeded and its SHA equals the remote branch head. It builds before interruption, stops the app for a consistent backup, starts the new version, verifies public health, and rolls back on failure.

Pause updates without stopping the app:
...
```sh
docker compose -p homelab -f /srv/ani-desk/source/deploy/homelab/compose.yml stop ani-desk
...
docker compose -p homelab -f /srv/ani-desk/source/deploy/homelab/compose.yml up -d ani-desk caddy
curl -fsS https://YOUR_DOMAIN/api/health
```
