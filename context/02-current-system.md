...
- `deploy/homelab/pull-deploy.sh` only accepts the latest successful GitHub `CI` push SHA on `master`, backs up stopped data, health-checks, and rolls back on failure.
